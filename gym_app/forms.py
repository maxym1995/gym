...
class AddMemberForm(forms.Form):
    ...
    year_of_birth = forms.IntegerField(
        label="Year of birth", min_value=1900, max_value=2022
    )
...
class AddUserForm(UserForm):
    password_1 = forms.CharField(
        widget=forms.PasswordInput, help_text="Type password twice"
    )
...
class AddRoomForm(forms.Form):
    ...
    training = forms.ModelChoiceField(
        label="Training type", queryset=Trainings.objects.all()
    )
...
class ReservationForm(forms.Form):
    ...
    training = forms.ModelChoiceField(
        ...
        queryset=Trainings.objects.all().order_by("date", "start_time"),
    )
