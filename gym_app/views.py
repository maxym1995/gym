...
class AddStaffView(PermissionRequiredMixin, LoginRequiredMixin, View):
    ...
    permission_required = (
        "gym_app.add_staff",
        "gym_app.view_staff",
        "gym_app.change_staff",
        "gym_app.delete_staff",
    )
    login_url = "/login/"
...
class ShowTrainings(View):
    ...
    def get(self, request):
        ...
        traning_list = zip(
            [(str(TRAININGS[int(t.name)][1])) for t in trainings],
            [(str(HOURS[int(st.start_time)][1])) for st in trainings],
            [(str(HOURS[int(et.end_time)][1])) for et in trainings],
            trainings,
        )
...
class AddRoomView(PermissionRequiredMixin, LoginRequiredMixin, View):
    ...
    permission_required = (
        "gym_app.add_rooms",
        "gym_app.change_rooms",
        "gym_app.delete_rooms",
    )
    login_url = "/login/"
...
class AddTrainerView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    permission_required = (
        "gym_app.add_trainers",
        "gym_app.view_trainers",
        "gym_app.change_trainers",
        "gym_app.delete_trainers",
    )
    login_url = "/login/"
        ...
        if form.is_valid():
            if form.cleaned_data["start_time"] > form.cleaned_data["end_time"]:
                return render(
                    request,
                    "training-add.html",
                    {"form": form, "error": "End time before start time"},
                )
            ...
            #     return render(request, "training-add.html", {"form":form, "error":"This trainer is already assigned to training."})
                ...
                return HttpResponse(
                    """Training has been added <br> <a href='/trainings'> Back</a> """
                )
...
class ReservationView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            return HttpResponse(
                """Reservation has been added <br> <a href='/reserve'> Back</a> """
            )
        return render(request, "main_page.html")
    ...
    def get(self, request):
        if request.user.is_authenticated:
            ...
            return render(
                ...
                {"reservations": reservations, "zipped": zipped},
            )
        else:
            return HttpResponse("""Login error <br> <a href=''> Back</a> """)
        return redirect(reverse("main"))
...
class UserReservationDeleteView(LoginRequiredMixin, View):
    ...
    def get(self, request, id):
        if request.user.is_authenticated:
            ...
            return HttpResponse(
                """Reservation has been deleted <br> <a href='/reservation_details/'> Back</a> """
            )
